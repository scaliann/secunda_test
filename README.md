## P.S.: Попытавшись реализовать все ручки отдельно, я пришел к тому, что в реальности использовать это было бы супер неудобно, поэтому решил сделать одну ручку с фильтрами, чтобы можно было комбинировать фильтры. Например, искать все компании, которые занимаются едой в определенном радиусе от точки и тд. Надеюсь, это не сильно страшно :) 

## P.P.S.: Чтобы посмотреть данные можно подключиться к бд. На локалхост она прокинута на 5430 порт.
...
*   **`activity_search_str` (string)**  
    **Поиск по названию деятельности.** Указывается название. В результатах будут организации, связанные как с самой указанной деятельностью, так и со всеми её дочерними элементами в дереве (вплоть до 3 уровня). Например, поиск по `Еда` найдёт организации с деятельностью "Еда", "Молочная продукция", "Сыры" и т.д.
...
http://localhost:8000/api/v1/organizations?building_ids=1&activity_ids=2,6

**Что проверить:** Результатом будут организации, которые **одновременно** находятся в здании с `id=1` И занимаются деятельностью с `id=2` **или** `id=6`. Пример использования нескольких фильтров для точного поиска.
